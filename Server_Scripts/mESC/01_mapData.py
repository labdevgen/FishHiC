...
genomeName = "mm10"
genome_db = genome.Genome("/mnt/storage/home/vsfishman/HiC/fasta/"+genomeName,readChrms=["#","X","Y"],
								)

bowtiePath = "/mnt/storage/home/vsfishman/HiC/bin/bowtie2/bowtie2"
bowtieIndex = "/mnt/storage/home/vsfishman/HiC/bin/bowtie2/index"+"/{0}".format(genomeName)  # change this if your index is named differently from the genome
...
GEOids = [1504838,1504839,1504844]
if mode=="sra":
	print "Using following SRA numbers:",GEOids
...
#           raise RuntimeError("Download failed with return code {0}".format(ret))
        print sra_folder+expName+".sra"
        counters = mapping.splitSRA(sra_folder+expName+".sra", os.path.join(fastqFolder, expName + "_chunk{0:04d}_fhtagn_side{1}.fastq.gz"), 
		chunkSize,FASTQ_BINARY="/mnt/storage/home/vsfishman/HiC/bin/sra/bin/fastq-dump")  # creating unique IDs requires creativity
#        os.remove(sraName)
