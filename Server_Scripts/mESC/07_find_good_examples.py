...
#hm_file = "/mnt/storage/home/vsfishman/HiC/data/mESC/mapped-mm10/mm10/mESC-all-HindIII_refined.frag"+str(resolution/1000)+"k_hiRes.hm.IC.cis.hdf5"
#hm_file = "/mnt/storage/home/vsfishman/HiC/data/mESC/mapped-mm10/mm10/mESC-all-HindIII_refined.frag"+str(resolution/1000)+"k_bychr.hm.IC.hdf5"
...
results_file.write("chr\tNucleotide\tMax_percentile\t"+"\t".join(map(str,sorted(distances.keys())))+"\n")

print "Calculating..."

result_counts=dict([(p,0) for p in percentiles])

for chr_numb,data in enumerate(get_chromosomes(hm_file,genome_db,resolution)):
	...
	for bin in xrange(max_distance+min_distance_from_chrm_end_in_bins,len(data)-max_distance-min_distance_from_chrm_end_in_bins):
		...
		if max_bin_percentile != 101: #if we satisfy any percentile for all distances
			...
			results_file.write("\t".join(map(str,[chr_numb,(bin*resolution)+resolution/2.,max_bin_percentile]+ratios))+"\n")
