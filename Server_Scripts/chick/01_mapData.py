...
inFastqDir = "/mnt/storage/home/vsfishman/tmp/Distr/Chick_HiC/new_data/Chicken_HiC-2/Project_Chic-HiC/fastq/"  # for mode="fastq" only

#sidePrefixes = ("side1", "side2")   # a version for naming ....side1.fastq.gz
sidePrefixes = ("R1_001","R2_001")  # a prefix preceeding .fastq.gz, which will be used to distinguish side 1 and side 2
# If your files are named "run32167_something_side1_somethingElse.fastq.gz", then "side1_somethingElse" should be the prefix. 

threads = 8
tmpDir = "/mnt/storage/home/vsfishman/tmp/HiC_tmp/"  # this will contain up to 3X the size of the largest input .sra file (256GB is enough for (Rao, 2014), but 128 is not)
...
genome_db = genome.Genome("/mnt/storage/home/vsfishman/HiC/fasta/galGal4/"+genomeName,readChrms=[],
								chrmFileTemplate="chr%s.fa",gapFile="galGal4.gap.txt")

bowtiePath = "/mnt/storage/home/vsfishman/HiC/bin/bowtie2/bowtie2"
bowtieIndex = "/mnt/storage/home/vsfishman/HiC/fasta/galGal4/"+genomeName+"/{0}".format(genomeName)  # change this if your index is named differently from the genome
...
if mode == "sra":
    ...
    iterList = 2 * sorted([i for i in os.listdir(inFastqDir) if "new_B1_TTAGGC_L001" in i])
