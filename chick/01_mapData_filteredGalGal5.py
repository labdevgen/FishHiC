...
inFastqDir = "/mnt/storage/home/vsfishman/tmp/Distr/Chick_HiC/Chicken_HiC-4/"  # for mode="fastq" only
#inFastqDir = "/mnt/storage/home/vsfishman/tmp/Distr/Chick_HiC/one_test_fastq/"  # for mode="fastq" only
...
threads = 10
tmpDir = "/mnt/storage/home/vsfishman/HiC_tmp/4/"  # this will contain up to 3X the size of the largest input .sra file (256GB is enough for (Rao, 2014), but 128 is not)
...
for i in  iterList:
    ...
    def doOne(inData,saveSams=True):
        ...
        for i in os.listdir(samFolder):
            if ((os.path.split(file1)[1] in i) or (os.path.split(file2)[1] in i)) and not saveSams:
                print("deleting", i)
